...
{ // best클래스 선언
    ...
    public function lists()
    {
        ...
        $base_url = "/best/lists/text1/$text1/text2/$text2/page";    // $page_segment = 6;
    ...
    public function view()
    {
        ...
        $data["page"]=$page;                                      // page 값 전달을 위한 처리
        $this->load->view("main_header"); // 상단출력(메뉴)
        $this->load->view("best_view", $data); // best_list에 자료전달
        $this->load->view("main_footer"); // 하단 출력
    }
    public function del()
    {
        ...
        $data["text1"]=$text1;                                      // text1 값 전달을 위한 처리
        $data["page"]=$page;                                      // page 값 전달을 위한 처리
        $this->best_m->deleterow($no);
        redirect("/~four1/best/lists/" . $text1 . $page); // 목록화면으로 돌아가기
    }
    public function add()
    {
        $uri_array=$this->uri->uri_to_assoc(3);
        ...
        {
            $data["list"] = $this->best_m->getlist_product();
            $this->load->view("main_header");
            $this->load->view("best_add", $data);    // 입력화면 표시
        ...
        else              // 입력화면의 저장버튼 클릭한 경우
        {
            ...
            $this->best_m->insertrow($data); 
    
            redirect("/~four1/best/lists/". $text1 . $page);    //   목록화면으로 이동.
        }
    ...
    public function edit()
    {
        ...
        if ($this->form_validation->run()==FALSE ) // 목록화면의 추가버튼 클릭한 경우
        {
            ...
            $data["row"] = $this->best_m->getrow($no);
            $this->load->view("main_header"); // 상단출력(메뉴)
            $this->load->view("best_edit", $data); // best_list에 자료전달
            $this->load->view("main_footer"); // 하단 출력
        }
        else              // 입력화면의 저장버튼 클릭한 경우
        {
